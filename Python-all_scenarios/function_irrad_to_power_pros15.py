...
def energy_offer(num_days, num_hours, num_seller, panels_num):
    ...
    P_mod_m2_Archal = P_stc * (G_Archal/G_stc) * (1 + c_temp * (Tc_Archal - Tc_stc)) #power generated per module -> its per panel not per m2
    #P_mod_Archal = P_mod_m2_Archal * A_mod #power generated by one module -> comment it and run it with the command below
