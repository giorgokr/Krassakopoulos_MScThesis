...
num_hours = 24
num_seller = 50 # declare the number of sellers
...
demand_F_array = [[0 for _ in range(num_hours)] for _ in range(num_days)] #final demand in kWh by consum to prosum only
supply_F_array = [[0 for _ in range(num_hours)] for _ in range(num_days)] #final supply in kWh by prosum to consum only
final_dem_ar = [[0 for _ in range(num_hours)] for _ in range(num_days)] #total final demand for this auction round by consumers to prosum and dso
final_sup_ar = [[0 for _ in range(num_hours)] for _ in range(num_days)] #total final supply for this auction round by prosumers to cons and dso
...
Price_EMA = [[0 for _ in range(num_hours)] for _ in range(num_days)] # array keeping the EMA prices in euro
...
for day in range(num_days):
    for hour in range(num_hours):
        ...
        #p_bal = 0.00012 * ether # = 0.20 euro, in  wei
        p_con = 0.00025 * ether # = 0.40 euro so that price in the range [0.10 , 0.90] euro, in wei
        #p_con = 0.000088 * ether # = 0.15 euro so that price in the range [0.05 , 0.35] euro, in wei
        ...
        """
        #find the available battery capacity for the auction 
